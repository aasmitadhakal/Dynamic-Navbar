...
function Navbar() {
  ...
  const fetchMenuData = async () => {
    try {
      const response = await axios.get(
        "https://sungavacms.paradiseit.com.np/api/menus/1"
      );
  ...
  const [hoveredItem3, setHoveredItem3] = useState("");
                              ...
                              {mainChild?.map((child, index) => {
                                return (
                                  <li
                                    ...
                                    onMouseLeave={() =>
                                      handleHoverOut2(child?.title)
                                    }
                                    className=" px-3 cursor-pointer relative hover:bg-gray-200 py-2 "
                                  >
                                    <Link to={child?.slug} className=" flex justify-between">
                                    ...
                                    {hoveredItem2 === child?.title && (
                                        <div className="absolute  left-44 w-44 top-[-3px]  bg-white ">
                                          <ul>
                                            {child &&
                                              child?.children &&
                                              child?.children[0] &&
                                              child?.children[0]?.map(
                                                (subChild, index) => {
                                                  return (
                                                    ...
                                                    key={child?.id}
                                                    onMouseEnter={() =>
                                                      handleHover3(child?.title)
                                                    }
                                                    onMouseLeave={() =>
                                                      handleHoverOut3(child?.title)
                                                    }
              ...
              <div className="flex flex-col items-center gap-4">
                {menu.map((menuItem, index) => (
                  ...
                  >
                    <Link
                      to={menuItem.slug}
                      className="flex relative mb-2 z-10 "
                    >
                      <span> {menuItem.title}</span>
                      {menuItem.children && (
                        <span className="text-gray-500  flex items-center">
